...
public:
	...
	Vector3u() { x = y = z = 0; }
	Vector3u(unsigned int x, unsigned int y, unsigned int z)
	{
		this->x = x;
		this->y = y;
		this->z = z;
...
public:
	...
	Vector4ub() { x = y = z = 0; }
	Vector4ub(uint8 x, uint8 y, uint8 z, uint8 w = 0)
	{
		this->x = x;
		this->y = y;
		this->z = z;
	...
	void set(uint8 x, uint8 y, uint8 z, uint8 w = 0)
	{
		this->x = x;
		this->y = y;
		this->z = z;
	...
	Vector4ub operator=(const Vector4 &a)
	{
		x = (uint8)a.x;
		y = (uint8)a.y;
		z = (uint8)a.z;
...
public:
	...
	Vector3 halfsize;
	BoundingBox() {}
	BoundingBox(Vector3 center, Vector3 halfsize)
	{
		this->center = center;
